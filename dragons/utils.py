...
def make_scare(action, terminator):
    ...
    action -- An action method of some Terminator
    """
    if not terminator.scared_turns_left:
        return action
    else:
        def new_action(colony):
            terminator.direction = 'Backward'
            action(colony)
            terminator.direction = 'Forward'
            terminator.scared_turns_left-=1;
